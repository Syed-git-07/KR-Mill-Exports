-- =====================================================
-- Breaker Drawing - Copy Previous Data Enhancement
-- =====================================================
...
-- This speeds up the getBreakerDrawingAvailableDates query
CREATE INDEX IF NOT EXISTS idx_bd_header_date_shift 
ON breaker_drawing_production_header(entry_date DESC, shift);

-- Create index on production_detail for faster header lookups
CREATE INDEX IF NOT EXISTS idx_bd_detail_header_id 
ON breaker_drawing_production_detail(header_id);

-- Create index on stoppage_entry for faster detail lookups
CREATE INDEX IF NOT EXISTS idx_bd_stoppage_detail_id 
ON breaker_drawing_stoppage_entry(production_detail_id);

-- =====================================================
-- Function to get available previous dates with data
-- =====================================================
CREATE OR REPLACE FUNCTION get_breaker_drawing_available_dates(
  p_before_date DATE,
  p_shift INTEGER,
...
-- Grant permissions
-- =====================================================
GRANT EXECUTE ON FUNCTION get_breaker_drawing_available_dates TO authenticated;

-- =====================================================
-- Verification
-- =====================================================
-- Test the function (uncomment to test)
-- SELECT * FROM get_breaker_drawing_available_dates('2025-12-25', 1, 10);

COMMENT ON FUNCTION get_breaker_drawing_available_dates IS 
