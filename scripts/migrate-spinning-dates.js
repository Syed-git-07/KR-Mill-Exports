...
async function run() {
  try {
    ...
    await prisma.$executeRaw`ALTER TABLE spinning_machines ADD COLUMN IF NOT EXISTS activated_at DATE NULL`;

    console.log('Adding deactivated_at column...');
    await prisma.$executeRaw`ALTER TABLE spinning_machines ADD COLUMN IF NOT EXISTS deactivated_at DATE NULL`;
    ...
    const active = await prisma.$executeRaw`
      ...
      WHERE activated_at IS NULL AND (is_active = 1 OR is_active IS NULL)
    `;
    console.log(`  Updated ${active} active machines`);

    console.log('Backfilling inactive machines...');
    const inactive = await prisma.$executeRaw`
      UPDATE spinning_machines
      SET activated_at = COALESCE(installed_date, DATE(created_at)),
          deactivated_at = CURDATE()
      WHERE activated_at IS NULL AND is_active = 0
    `;
