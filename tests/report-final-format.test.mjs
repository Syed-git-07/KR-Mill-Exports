...
test('critical report calculations use the corrected production rules', async () => {
  ...
  assert.match(finalReports, /async function autoconerShiftProduction[\s\S]*?const orderedGroups = \[\.\.\.groups\.entries\(\)\]\.sort[\s\S]*?Number\(leftShift\) - Number\(rightShift\)[\s\S]*?for \(const \[key, rows\] of orderedGroups\)/)
...
test('spinning and autoconer reports retain every count run and aggregate stored values', async () => {
  ...
  assert.match(siderMonthly, /new Date\(`\$\{fromDateKey\}T00:00:00\.000Z`\)/)
  assert.match(siderMonthly, /entry_date: \{ gte: databaseFromDate, lte: databaseToDate \}/)
