...
import { generatePreparatoryStoppageReportAction } from '@/app/actions/preparatory-reports'
import { jsPDF } from 'jspdf'
import autoTable from 'jspdf-autotable'

/**
 * Preparatory Stoppage Percentage Report Page
 * Displays stoppage analysis by department, category, and shift
 */
export default function PreparatoryStoppageReportPage() {
  const [fromDate, setFromDate] = useState(new Date())
  const [toDate, setToDate] = useState(new Date())
  const [reportData, setReportData] = useState(null)
  const [isLoading, setIsLoading] = useState(false)

  // Category prefix mapping for display
  const CATEGORY_PREFIX = {
    'Cleaning Work': 'CL',
    'Electrical Breakdown': 'EB',
    'Maintenance Breakdown': 'MB',
    'Maintenance Routine': 'MR',
    'Others': 'OT'
  }
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      doc.setFontSize(14)
      doc.text('Stoppage Percentage Report — Preparatory', pageWidth / 2, yPosition, { align: 'center' })
      yPosition += 6
      ...
      Object.entries(reportData.departments).forEach(([deptName, deptData], deptIndex) => {
        ...
        Object.entries(deptData.categories).forEach(([category, categoryData]) => {
          ...
          const tableData = categoryData.reasons.map((reasonData, idx) => {
            ...
            return [
              ...
              reasonData.shift3.toFixed(2),
              reasonData.total.toFixed(2)
            ]
          })
          ...
          tableData.push([
            '',
            'TOTAL',
            categoryData.categoryTotal.shift1.toFixed(2),
            categoryData.categoryTotal.shift2.toFixed(2),
            categoryData.categoryTotal.shift3.toFixed(2),
            categoryData.categoryTotal.total.toFixed(2)
          ])

          // Generate table
          autoTable(doc, {
            startY: yPosition,
            head: [['SL No', 'Reason', 'I Shift %', 'II Shift %', 'III Shift %', 'Total %']],
            body: tableData,
            theme: 'grid',
            headStyles: {
      ...
      const filename = `Preparatory_Stoppage_Report_${fromDateStr}_to_${toDateStr}.pdf`
        ...
        <Card className="bg-linear-to-r from-blue-600 to-blue-700 text-white">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-2xl">Preparatory Stoppage Percentage Report</CardTitle>
                <p className="text-blue-100 mt-2">
                  Analyze stoppage patterns across preparatory departments
                </p>
              </div>
              <Link href="/reports">
          ...
          <CardContent className="pt-6">
            <div className="space-y-8">
              ...
              <div className="text-center border-b pb-4 print:border-black">
                ...
                <h2 className="text-xl font-semibold mt-2">Stoppage Percentage Report — Preparatory</h2>
              ...
              {/* Departments */}
              {Object.entries(reportData.departments).map(([deptName, deptData]) => (
                <div key={deptName} className="space-y-4">
                  <h3 className="text-lg font-bold uppercase border-b-2 border-gray-300 pb-2 print:border-black">
                    {deptName}
                  </h3>
                        ...
                        <table className="w-full border-collapse text-sm">
                          <thead>
                            <tr className="bg-gray-100 print:bg-gray-200">
                              <th className="border border-gray-300 px-2 py-1 text-left w-16 print:border-black">SL No</th>
                              <th className="border border-gray-300 px-2 py-1 text-left print:border-black">Reason</th>
                              <th className="border border-gray-300 px-2 py-1 text-right w-24 print:border-black">I Shift %</th>
                              <th className="border border-gray-300 px-2 py-1 text-right w-24 print:border-black">II Shift %</th>
                              <th className="border border-gray-300 px-2 py-1 text-right w-24 print:border-black">III Shift %</th>
                              <th className="border border-gray-300 px-2 py-1 text-right w-24 print:border-black">Total %</th>
                            </tr>
                          </thead>
                          <tbody>
                            {categoryData.reasons.map((reasonData, idx) => {
                              // Check if reason already has prefix, if not add it
                              const displayReason = reasonData.reason.startsWith(prefix) 
                                ? reasonData.reason 
                                : `${prefix} ${reasonData.reason}`
                            ...
                            <tr className="bg-gray-50 font-bold print:bg-gray-100">
                              <td className="border border-gray-300 px-2 py-1 print:border-black"></td>
                              <td className="border border-gray-300 px-2 py-1 print:border-black">TOTAL</td>
                              <td className="border border-gray-300 px-2 py-1 text-right print:border-black">
                                {categoryData.categoryTotal.shift1.toFixed(2)}
                              </td>
                              <td className="border border-gray-300 px-2 py-1 text-right print:border-black">
