...
import {
  getDrawingBreakerMachines,
  createDrawingBreakerMachine,
  updateDrawingBreakerMachine,
  deleteDrawingBreakerMachine,
  searchDrawingBreakerMachines
} from '@/lib/supabase/drawingBreakerQueries';
...
export default function DrawingBreakerPage() {
  ...
  // VB6 Grid columns: McNo, Mixing Name, Description, Make, Speed
  const columns = [
    { key: 'machine_no', label: 'McNo', width: '100px' },
    { key: 'prodn_mixing', label: 'Mixing Name', width: '100px' },
  ...
  const loadMachines = async () => {
    try {
      setLoading(true);
      const data = await getDrawingBreakerMachines();
  ...
  const handleSearch = async (field, condition, value) => {
    ...
    try {
      const data = await searchDrawingBreakerMachines(field, condition, value);
  ...
  const handleDelete = async () => {
    if (isSelectMode && selectedRows.length > 0) {
      ...
      try {
        await Promise.all(selectedRows.map(row => deleteDrawingBreakerMachine(row.id)));
      ...
      try {
        await deleteDrawingBreakerMachine(selectedMachine.id);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (isEditing && selectedMachine) {
        await updateDrawingBreakerMachine(selectedMachine.id, formData);
        toast.success('Machine updated successfully');
      } else {
        await createDrawingBreakerMachine(formData);
  ...
  return (
    <div className="container mx-auto p-3 sm:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">Drawing Breaker Machine Master</h1>
